...
“transcribed” means that source logic was translated into native R; and
“native adaptation” means that the package implements an upstream algorithm
in a modified native form while still delegating any named numerical fit. No
source translation is claimed unless it is stated explicitly.

| Component | Package implementation | Statistical or algorithmic source | Software source and relationship |
|---|---|---|---|
| Counterfactual distributions, structure/composition effects, functional inference | `R/decomposition_engine.R`, `R/bootstrap.R`, `R/inference.R`, `R/functional_tests.R` | Chernozhukov, Fernández-Val, and Melly (2013), [doi:10.3982/ECTA10582](https://doi.org/10.3982/ECTA10582) | The estimands and bootstrap organization follow the paper and CRAN `Counterfactual` 1.2; the package is a new modular implementation, not a source copy. |
| Linear quantile regression objective | `R/solver_qr.R`, `R/conditional_model.R` | Koenker and Bassett (1978), [doi:10.2307/1913643](https://doi.org/10.2307/1913643) | Exact numerical fits are delegated to public routines in CRAN [`quantreg`](https://cran.r-project.org/package=quantreg). Version 6.1 was used for release validation. |
...
| Quantile and marginal-CDF rearrangement | `R/marginal_distribution.R` | Chernozhukov, Fernández-Val, and Galichon (2010), [doi:10.3982/ECTA7880](https://doi.org/10.3982/ECTA7880) | Row-wise QR/CQR quantile rearrangement and increasing rearrangement of the distribution-regression marginal CDF over ordered outcome thresholds are implemented in this package. |
| QR residual-sign bootstrap preprocessing (`xy_preprocess`) | `R/bootstrap.R` | Algorithm exposed by CRAN `quantreg` | Native adaptation of the `quantreg::boot.rq.pxy()` preprocessing algorithm followed by delegated fitting through `quantreg::rq.wfit()`; no `quantreg` source is copied verbatim. |
